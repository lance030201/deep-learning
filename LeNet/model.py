...
def random_flip(batch_data):
    #帮助随机翻转一批图像
    flip_batch = np.zeros(len(batch_data) * 28 * 28 * 1).reshape(len(batch_data), 28, 28, 1)
#    print(batch_data[0].shape)
#    print(flip_batch.shape)
...
# =============================================================================
#         self.augmentation_in = tf.placeholder(tf.float32, [None, 28, 28, 1])
...
# =============================================================================
        ...
        self.imgs = tf.placeholder(tf.float32, [None, 28, 28, 1])
        ...
        with tf.name_scope('conv1') as scope:
            kernel = tf.Variable(tf.truncated_normal([5, 5, 1, model_info['conv1']], dtype=tf.float32,
...
#            self.conv1_1 = tf.nn.dropout(self.conv1_1, self.keep_prob_conv_1)
        ...
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.fc2l, labels=self.labs))
...
#                image_data = random_flip(image_data)
    ...
    def writeInfoToTxt(self, path=r'.\model\log.txt'):
